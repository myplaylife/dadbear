package me.littlepanda.dadbear.core.berkeleydb;  
...
public class BdbPersistentQueue<E extends Serializable> extends AbstractQueue<E> implements  
	...
	private static Log log = LogFactory.getLog(BdbPersistentQueue.class);
    ...
    public boolean offer(E e) {  
        synchronized(tailIndex){  
            queueMap.put(tailIndex.getAndIncrement(), e);   // 浠庡熬閮ㄦ彃鍏� 
        }  
        return true;  
    }  
    ...
    public E peek() {  
        synchronized(headIndex){  
            ...
            E headItem=null;  
            while(headItem==null&&headIndex.get()<tailIndex.get()){ // 娌℃湁瓒呭嚭鑼冨�? 
                headItem=queueMap.get(headIndex.get());  
                if(headItem!=null){  
                    peekItem=headItem;  
                    continue;  
                }   
                headIndex.incrementAndGet();    // 澶撮儴鎸囬拡鍚庣Щ  
            }  
            return headItem;  
        }  
    }  
    ...
    @Override  
    public void clear() {
    	throw new RuntimeException("no clear db!");
//       try {  
//           close();  
